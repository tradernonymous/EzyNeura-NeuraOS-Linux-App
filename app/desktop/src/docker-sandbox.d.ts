...
export type SandboxBinary = 'docker' | 'podman';
...
export declare const DEFAULT_BINARY: SandboxBinary;
/** The container runtimes this sandbox can drive, in try-first order. */
export declare const BINARIES: SandboxBinary[];
...
export declare function wrap(opts: { root: string; command: string; cwd?: string; image?: string; binary?: SandboxBinary; id?: string }):
...
/** Try each of BINARIES in turn against `runner` and return the first whose
 * daemon/socket answers, or null when neither does. */
export declare function detectBinary<R extends RunResultLike>(runner: Runner<R>): Promise<SandboxBinary | null>;
