...
export default function HfSignIn({ compact, showClientId, onSignedIn }: Props) {
  ...
  if (waiting) {
    ...
    body = clientId ? (
      <>
        {!compact && <span>Sign in once in your browser. The token stays on this PC, in {credentialStoreName()}.</span>}
      ...
      <>
        {!compact && <span>Hugging Face needs a token. It stays on this PC, in {credentialStoreName()}.</span>}
