...
(function (root, factory) {
  ...
  /** Double-quote a path; quotes inside are refused, not escaped. Safe on
   * both cmd.exe and a POSIX shell (sh/bash) because slug() only ever
   * produces `[a-z0-9-]`, so there is never a `$`, backtick or backslash
   * inside the quotes for either shell to expand. */
  function q(path) {
    var p = String(path);
