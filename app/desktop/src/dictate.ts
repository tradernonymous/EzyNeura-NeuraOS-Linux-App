...
import './wav.js';
import { builtInDictationSentence } from './platform';
...
export function noEngineMessage(setting: EngineSetting): string {
  const fallback = builtInDictationSentence();
  if (setting === 'local') return `Local dictation needs whisper.cpp and a ggml model: set them up under Settings → Dictation. ${fallback}`;
  if (setting === 'hf') return `Dictation uses Whisper on Hugging Face: sign in under Settings → Connectors. ${fallback}`;
  return `Dictation needs whisper.cpp (Settings → Dictation) or a Hugging Face sign-in (Settings → Connectors). ${fallback}`;
}
