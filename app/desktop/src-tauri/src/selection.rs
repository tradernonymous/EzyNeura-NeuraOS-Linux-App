...
// Linux has no Ctrl+C-then-read-clipboard trick to port: X11 and Wayland
// both already keep the highlighted text in the PRIMARY selection, updated
// the instant text is highlighted -- no hotkey, no keybd_event, and the
// CLIPBOARD selection (what a real Ctrl+C would set) is never touched, so a
// press of this hotkey can never clobber something the person meant to
// paste.
#[cfg(target_os = "linux")]
fn copy_selection() -> Option<String> {
