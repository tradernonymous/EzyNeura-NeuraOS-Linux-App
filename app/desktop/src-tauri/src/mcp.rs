...
fn kill_tree(child: &mut Child) {
    ...
    // `npx` launches node as a child of itself, so the process group started
    // for it below (local.rs W4) is what makes this reach both.
    #[cfg(unix)]
    {
        let group = format!("-{}", child.id());
        let _ = Command::new("kill").args(["-TERM", &group]).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).status();
...
pub fn mcp_stdio_start(
    ...
    // Its own process group, so kill_tree() above reaches whatever this
    // command starts (`npx` launching node) and not just the launcher.
    #[cfg(unix)]
    {
