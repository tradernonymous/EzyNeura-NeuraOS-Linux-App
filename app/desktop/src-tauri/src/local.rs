...
pub const RISKY: &[(&str, &str)] = &[
    ...
    // Linux additions (docs/MASTER_PLAN.md, W6): the Windows list above has
    // no equivalent for a root shell, a raw-disk write, or a systemd/apt
    // action, and none of these strings occur in ordinary Windows commands,
    // so they cost that platform nothing.
    ("sudo ", "asks for administrator rights"),
    ("pkexec", "asks for administrator rights"),
    ("dd if=", "writes raw bytes to a device or file, bypassing the filesystem"),
    ("mkfs", "formats a disk"),
    ("wipefs", "erases a disk's filesystem signatures"),
    ("chmod -r 777", "opens a whole tree to every user"),
    ("chown -r", "changes ownership of a whole tree"),
    ("rm -rf ~", "deletes the home directory"),
    ("rm -rf /", "deletes the root of the filesystem"),
    ("rm -rf --no-preserve-root", "deletes the root of the filesystem"),
    ("systemctl", "starts, stops or reconfigures a system service"),
    ("apt purge", "removes a package and its configuration"),
    ("apt-get purge", "removes a package and its configuration"),
    ("apt remove", "removes an installed package"),
    ("apt-get remove", "removes an installed package"),
    ("wget | sh", "pipes a download into a shell"),
...
fn kill_tree(child: &mut std::process::Child) {
    ...
    // shell_command() below puts this child in its own process group (pgid
    // == its own pid), so signalling the negative pid reaches it and every
    // process it started -- the same "npm test is two processes" problem
    // Windows has, solved the Unix way instead of by walking the process
    // tree by hand (W4, docs/MASTER_PLAN.md section 2).
    #[cfg(unix)]
    {
...
// `sh -c` has no `.bashrc`, nvm or pyenv in its PATH: a GUI app is not
// launched from a login shell, so `node`/`cargo`/`python` under `~/.local/bin`
// or a version manager's shims are invisible to it even though a real
// terminal on the same machine sees them (W5, docs/MASTER_PLAN.md section
...
fn shell_command(command: &str) -> std::process::Command {
    ...
    if shell.file_name().and_then(|n| n.to_str()) == Some("sh") {
        cmd.args(["-c", command]);
    } else {
        cmd.args(["-lc", command]);
    }
    // Its own process group (pgid == this process's own pid): kill_tree()
    // above signals the negative pid to reach every process this command
    // starts, not just the shell itself.
    cmd.process_group(0);
    cmd
}
