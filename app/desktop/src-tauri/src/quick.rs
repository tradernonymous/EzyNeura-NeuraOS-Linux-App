...
// Alt+Space is Cinnamon's (and most other Linux desktops') own window-menu
// shortcut, so asking for it here would either silently lose to the desktop
// or silently steal it back -- neither is a hotkey the person chose. Settings
// still offers to remap it, same as every other platform.
#[cfg(target_os = "linux")]
