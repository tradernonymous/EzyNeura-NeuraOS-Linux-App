...
//
// Mica is a Windows 11 / DWM concept with nothing to port: Linux has no
// system-wide translucent backdrop tauri.conf.json could ask a compositor
// for in the same way, so window_has_mica() below answers `false` on every
// other platform and the shell keeps its own flat background there (the
// Linux equivalent -- following the desktop's accent colour and light/dark
// setting -- is theme.ts's job, not this module's).
#[cfg(windows)]
mod mica {
...
fn main() {
    ...
    // WebKitGTK's GPU-accelerated DMA-BUF renderer leaves the window blank
    // on the NVIDIA proprietary driver, and on some Wayland compositors, on
    // NeuraOS's own machine among many other Tauri apps' bug trackers
    // (docs/MASTER_PLAN.md section 2). This has to be set before the first
    // webview is created, so it runs here, first thing, rather than in
    // .setup() which runs after the window exists.
    #[cfg(target_os = "linux")]
    ...
    builder
        .invoke_handler(generate_handler![
            ...
            git::local_git_amend,
            git::local_git_unstage,
            git::local_git_discard,
            git::local_git_push,
            git::local_git_pull,
