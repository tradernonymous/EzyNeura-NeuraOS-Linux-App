...
test('every accent token is derived from --accent-h, in both themes', () => {
  assert.match(DARK, /--accent-h:\s*286;/, 'the default hue (Neural Violet) is set on :root');
...
test('the hue is stored under freeai4u.accent_hue and painted before the first frame', async () => {
  ...
  try {
    // Linux port (docs/MASTER_PLAN.md section 4): the default accent is
    // Neural Violet (hue 286), matching the Android app, not upstream's
    // green (152) -- green is still offered as its own preset.
    assert.equal(theme.readAccentHue(), 286, 'no choice: the default');
    ...
    assert.equal(theme.readAccentHue(), 286, 'garbage falls back to the default');
    theme.applyAccentHue(null);
    assert.equal(store.has('freeai4u.accent_hue'), false, 'reset forgets the choice');
    assert.equal(props.get('--accent-h'), '286', 'and paints the default');
...
test('Settings has an Appearance card with a hue slider, six presets and Reset', () => {
  ...
  // Linux port: "Neural Violet" is two capitalised words (the Android
  // app's own name for it), where upstream's presets were all one word;
  // green moved off the default slot to its own preset, so the count (six)
  // is unchanged from upstream.
  assert.equal((CARD.match(/\{ name: '[A-Z][a-zA-Z ]+', hue: /g) || []).length, 6, 'six presets');
  assert.match(CARD, /name: 'Neural Violet', hue: DEFAULT_ACCENT_HUE/, 'Neural Violet is the default preset');
